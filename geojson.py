...
#The program takes the search string and constructs a URL with the search string as a properly encoded parameter
    ...
    print(urllib.parse.urlencode(parms)) # me devuelve ---> address=ann+arbor&key=42  . Ver https://docs.python.org/3/library/urllib.parse.html#urllib.parse.urlencode
...
#then extract the information that we are looking for.
    ...
    if not js or 'status' not in js or js['status'] != 'OK':    #nose porque status pero es para chequeo de que lo que obtuve esta ok
